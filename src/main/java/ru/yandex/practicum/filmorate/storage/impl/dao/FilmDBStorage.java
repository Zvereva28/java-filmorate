...
public class FilmDBStorage implements FilmStorage {
    ...
    public Film updateFilm(Film film) {
        ...
        Integer countLikes;
        try {
            String SELECT_COUNT_OF_LIKES = "SELECT count(*) AS count FROM film_likes where film_id = ?";
            countLikes = jdbcTemplate.queryForObject(SELECT_COUNT_OF_LIKES,
                    (rs, rowNum) -> rs.getInt("count"), film.getId());
        } catch (RuntimeException e) {
            countLikes = 0;
        }
        String UPDATE_FILM = "UPDATE films SET  name=?, description=?, release_date=?, duration=?, rating_mpa=?, count_likes=? WHERE id=?";
        ...
        String DELETE_FILM_GENRE = "DELETE FROM film_genre WHERE film_id=?";
    ...
    public List<Film> getAllFilms() {
        try {
            String SELECT_ALL_FILMS = "SELECT f.id, name, description, release_date, duration, rating_mpa, count_likes, fg.genre_id AS genre_id, g.genre_name AS genre_name " +
    ...
    public void removeLike(Integer filmId, Integer userId) {
        String DELETE_LIKES = "DELETE FROM film_likes WHERE film_id=? AND user_id=?";
    ...
    private Film filmExist(int id) {
        String SELECT_FILM = "SELECT f.id, name, description, release_date, duration, rating_mpa, count_likes, fg.genre_id AS genre_id, g.genre_name AS genre_name " +
                "FROM films as f LEFT JOIN film_genre AS fg ON f.id=fg.film_id LEFT JOIN genre AS g ON fg.genre_id=g.id " +
